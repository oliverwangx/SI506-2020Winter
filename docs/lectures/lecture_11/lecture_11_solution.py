...
def count_tour_dates_by_location(tour_dates, location='venue'):
    ...
    for tour_date in tour_dates:
        for key, val in tour_date.items():
            if key == location:
...
def get_locations(tour_dates, location='venue'):
    ...
    locations = []
    for tour_date in tour_dates:
        place = format_location(tour_date, location) # call function
...
def get_venues_by_seating(tour_dates, min_seats, max_seats=None):
    ...
    for tour_date in tour_dates:
        ...
        if (venue not in venues
            and min_seats <= venue['seating'] <= max_seats):
...
def main():
    ...
    # 1.0 ACCESSING LIST DICTIONARY ELEMENTS
